...

{
  ...
  bintoolsNoLibc = wrapBintoolsWith {
    ...
    libc = if stdenv.targetPlatform.useLLVM
      then null
      else preLibcCrossHeaders;
  };
  ...
  preLibcChooser = platform: pkgs: fallback: let
    inherit (platform) libc;
  in     if platform.isMinGW then pkgs.windows.mingw_w64_headers or fallback
    else if libc == "fblibc" then pkgs.freebsd.include or fallback
    else if libc == "nblibc" then pkgs.netbsd.headers or fallback
    else if libc == "slibc" then pkgs.illumos.headers or fallback
    else null;
  ...
  # We can choose:
  libcChooser = platform: pkgs: fallback: let
    inherit (platform) libc;
    # libc is hackily often used from the previous stage. This `or`
    # hack fixes the hack, *sigh*.
  in     if libc == null then null
    else if libc == "glibc" then pkgs.glibcCross or fallback
    else if libc == "bionic" then pkgs.bionic or fallback
    else if libc == "uclibc" then pkgs.uclibc or fallback
    else if libc == "avrlibc" then pkgs.avrlibc or fallback
    else if libc == "newlib" && platform.isMsp430 then pkgs.msp430Newlib or fallback
    else if libc == "newlib" && platform.isVc4 then pkgs.vc4-newlib or fallback
    else if libc == "newlib" && platform.isOr1k then pkgs.or1k-newlib or fallback
    else if libc == "newlib" then pkgs.newlib or fallback
    else if libc == "newlib-nano" then pkgs.newlib-nano or fallback
    else if libc == "musl" then pkgs.muslCross or fallback
    else if libc == "msvcrt" then pkgs.windows.mingw_w64 or fallback
    else if libc == "ucrt" then pkgs.windows.mingw_w64 or fallback
    else if libc == "libSystem" then
      if platform.useiOSPrebuilt
      then pkgs.darwin.iosSdkPkgs.libraries or fallback
      else pkgs.darwin.libSystem or fallback
    else if libc == "fblibc" then pkgs.freebsd.libc or fallback
    else if libc == "oblibc" then pkgs.openbsd.libc or fallback
    else if libc == "nblibc" then pkgs.netbsd.libc or fallback
    #else if libc == "slibc" then pkgs.illumos.libc or fallback
    else if libc == "wasilibc" then pkgs.wasilibc or fallback
    else if libc == "relibc" then pkgs.relibc or fallback
    else throw "Unknown libc ${libc}";

  libcCross =
    if stdenv.targetPlatform == stdenv.buildPlatform
    then null
    else libcChooser stdenv.targetPlatform targetPackages
