...
, # Use to reevaluate Nixpkgs; a dirty hack that should be removed
  ...
  # stage becomes the build platform of the current one, and likewise the target
  # platform of the previous stage becomes the host platform of the current one.
  #
...
let
  ...
  in {
    ...
    crossSystem = if hostPlatform != buildPlatform then hostPlatform else null;
  };
