...
# kernel path.
#
# Built from `usr/src/cmd/ctfconvert`, through the gate's own makefile, under
# dmake -- like every other package in this set. Not through
# `usr/src/tools/ctf/ctfconvert`, which holds no source: it is a `%.o:
# $(SRC)/cmd/ctfconvert/%.c` rule and a makefile saying "build this one for the
...
# `buildPackages.illumos.ctfconvert` is already an instance whose `stdenv`
# targets the build machine, so there is nothing to arrange: consumers put
# `ctfconvert` in `nativeBuildInputs` and splicing hands them that instance
