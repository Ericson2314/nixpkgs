...
# ctfconvert(1ONBLD): turn a compiler's DWARF into CTF. `genunix` cannot be
# built without it, and `unix` links against `libgenunix.so`, so this gates the
# whole kernel path.
#
...
mkDerivation {
  ...
  path = "usr/src/tools/ctf/ctfconvert/i386";
  extraPaths = [
  ...
  # expects a plain `ctfconvert` on $PATH.
  postFixup = ''
    ln -s i386/ctfconvert $out/bin/ctfconvert
  '';
