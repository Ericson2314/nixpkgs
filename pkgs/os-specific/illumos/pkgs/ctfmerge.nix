...
# ctfmerge(1ONBLD): merge per-object CTF containers into one, and write it back
# into the linked object. The other half of the CTF pipeline; see ctfconvert.nix.
#
...
mkDerivation {
  ...
  path = "usr/src/tools/ctf/ctfmerge/i386";
  extraPaths = [
  ...
  # These are the build-platform spellings of NIX_LDFLAGS.
  ...
  # expects a plain `ctfmerge` on $PATH.
  postFixup = ''
    ln -s i386/ctfmerge $out/bin/ctfmerge
  '';
