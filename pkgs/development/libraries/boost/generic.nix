...
stdenv.mkDerivation {
  ...
  passthru = {
    ...
    #
    # The patch only touches `gcc.jam`'s `# sun` block, so it would be inert
    # for every other target; it is scoped only so that `boost-build` does not
    # rebuild for them. TODO on staging, drop the `lib.optional` and apply it
